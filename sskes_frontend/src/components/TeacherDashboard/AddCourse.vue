      ...
      <form @submit.prevent="submitForm" enctype=“multipart/form-data”>
        ...
        <div class="col-md-12">
          <div class="mb-3">
            ...
            <textarea name="description" id="description" v-model="short_description" class="form-control sinhala-font"></textarea>
          </div>
        </div>
        <div class="col-md-12">
          <div class="mb-3">
            <label for="name" class="form-label sinhala-font">ඉගෙනුම ලබන වසර සදහා</label>
            <select name="selectGrade" id="CourseGrade" v-model="grade" class="form-select form-select-lg sinhala-font bg-light float-start mb-3" aria-label=".form-select-lg ">
    ...
    submitForm(){
      ...
      this.errors = []
        ...
        if(!this.errors.length){
            ...
            let x = 'abc'
            let newName = x+''+this.title
            fd.append('image', this.selectedFile, this.selectedFile.name)
            fd.append('title', newName)
          ...
          //  formData.is_student = this.is_student;
           ...
           let accessToken = localStorage.getItem('accessToken')
            axios
                .post('api/v1/courseRegister/', fd, { headers: { "Content-Type": "multipart/form-data", Authorization: `Bearer ${accessToken}` } })
                .then(response =>{
                    console.log(response.data)
                   // this.response = response.data
                   // this.response = response.data

                    this.success.push('Course Register successfully!')
